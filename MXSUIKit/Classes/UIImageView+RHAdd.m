...
static const CGFloat rh_FontResizingProportion = 0.42f;

@implementation UIImageView (RHAdd)
...
+ (id)rh_imageViewWithImageArray:(NSArray *)imageArray duration:(NSTimeInterval)duration;
{
    ...
    UIImageView *imageView = [UIImageView rh_imageViewWithImageNamed:[imageArray objectAtIndex:0]];
...
- (CGRect)rh_convertRectFromImage:(CGRect)imageRect {
    ...
    CGPoint viewTopLeft     = [self rh_convertPointFromImage:imageTopLeft];
    CGPoint viewBottomRight = [self rh_convertPointFromImage:imageBottomRight];
...
- (CGRect)rh_convertRectFromView:(CGRect)viewRect {
    ...
    CGPoint imageTopLeft = [self rh_convertPointFromView:viewTopLeft];
    CGPoint imageBottomRight = [self rh_convertPointFromView:viewBottomRight];
...
- (void)rh_setImageWithString:(NSString *)string {
    [self rh_setImageWithString:string color:nil circular:NO textAttributes:nil];
}

- (void)rh_setImageWithString:(NSString *)string color:(UIColor *)color {
    [self rh_setImageWithString:string color:color circular:NO textAttributes:nil];
}

- (void)rh_setImageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular {
    [self rh_setImageWithString:string color:color circular:isCircular textAttributes:nil];
}

- (void)rh_setImageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular fontName:(NSString *)fontName {
    [self rh_setImageWithString:string color:color circular:isCircular textAttributes:@{
                                                                                        NSFontAttributeName:[self rh_fontForFontName:fontName],
...
- (void)rh_setImageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)textAttributes {
    if (!textAttributes) {
        textAttributes = @{
                           NSFontAttributeName: [self rh_fontForFontName:nil],
                           NSForegroundColorAttributeName: [UIColor whiteColor]
                           };
    }
    ...
    UIColor *backgroundColor = color ? color : [self rh_letter_randomColor];
    
    self.image = [self rh_imageSnapshotFromText:[displayString uppercaseString] backgroundColor:backgroundColor circular:isCircular textAttributes:textAttributes];
}

#pragma mark - Helpers

- (UIFont *)rh_fontForFontName:(NSString *)fontName {
    
    CGFloat fontSize = CGRectGetWidth(self.bounds) * rh_FontResizingProportion;
    if (fontName) {
...
- (UIImage *)rh_imageSnapshotFromText:(NSString *)text backgroundColor:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)textAttributes {
